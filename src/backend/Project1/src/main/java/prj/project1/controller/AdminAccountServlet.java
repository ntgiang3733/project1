...
public class AdminAccountServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        long users_ID=(long)Long.parseLong(id);
        ud.delete(users_ID);
        String url="/admin/ad-account.jsp";
       ...
       String url="/register.jsp";
        try {
            if(name_err.length()==0&&email_err.length()==0&&password_err.length()==0){
                Date date=new Date();
                Users users=new Users(date.getTime(),name,email,password,true);
                ud.insertUser(users);
              
                url="/admin/ad-account.jsp";
            }else{
                url="/admin/ad-account.jsp";
            }
            RequestDispatcher rd= getServletContext().getRequestDispatcher(url);
